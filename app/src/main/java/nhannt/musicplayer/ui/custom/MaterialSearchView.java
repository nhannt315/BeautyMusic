...
import nhannt.musicplayer.utils.AnimationUtils;
...
public class MaterialSearchView extends FrameLayout {
    ...
    private void initiateView() {
        ...
        mSuggestionsRv = (RecyclerView) mSearchLayout.findViewById(R.id.suggestion_list);
        ...
        mSearchSrcTextView.setPadding(mBackBtn.getDrawable().getMinimumWidth() * 2, 0, mSearchSrcTextView.getPaddingRight(), 0);

        showVoice(true);

        initSearchView();

        mSuggestionsRv.setVisibility(GONE);
...
//            view.clearFocus();
    ...
    /**
     * Call this method to show suggestions list. This shows up when adapter is set. Call {@setAdapter(Adapter mAdapter)} before calling this.
     */
    public void showSuggestions() {
        if (mAdapter != null && mAdapter.getItemCount() > 0 && mSuggestionsRv.getVisibility() == GONE) {
            mSuggestionsRv.setVisibility(VISIBLE);
        }
    }
    ...
    public void dismissSuggestions() {
        mAdapter = null;
        if (mSuggestionsRv.getVisibility() == VISIBLE) {
            mSuggestionsRv.setVisibility(GONE);
        }
    }
    ...
    private void setVisibleWithAnimation() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            AnimationUtils.reveal(mSearchLayout, animationListener);
    ...
    public void closeSearch() {
        ...
        if (Common.isLollipop()) {
            mSearchLayout.setVisibility(View.VISIBLE);
            AnimationUtils.revealHide(mContext, mSearchLayout, animationListener);

        } else {
            AnimationUtils.fadeOutView(mSearchLayout, mAnimationDuration, animationListener);
        }
